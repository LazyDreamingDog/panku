...
type TxData interface {
	...
	// Change TODO: 修改AL 已在五个交易类型中实现了Change方法
	Change(MsgTAL []AccessTuple) bool
}

// ChangeAccessList TODO: 修改用户交易tx的AccessList
func (tx *Transaction) ChangeAccessList(MsgTAL []AccessTuple) bool {
	return tx.inner.Change(MsgTAL)
}
...
func (tx *Transaction) Hash() common.Hash {
	...
	if tx.Type() == LegacyTxType {
		h = rlpHash(tx.inner)
	} else if tx.Type() == PankuTxType {
		// when we hash panku tx, the StrictAccessList should be nil
		inner := new(PankuTx)
		inner = (tx.inner.copy()).(*PankuTx)
		inner.StrictAccessList = nil

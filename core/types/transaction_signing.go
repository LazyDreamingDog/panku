...
func MakeSigner(config *params.ChainConfig, blockNumber *big.Int, blockTime uint64) Signer {
	...
	// }
	signer = NewPankuSigner(config.ChainID)
	return signer
}

// TODO: LatestSigner返回给定链配置中可用的“最宽容”的Signer
// 具体来说，当它们各自的分叉计划在链配置中的任何块号（或时间）发生时，这支持所有类型的交易

// 在处理交易的代码中使用此方法，其中当前块号是未知的。如果您有可用的当前块号，请使用MakeSigner代替。
...
type pankuSigner struct{ cancunSigner }

func NewPankuSigner(chainId *big.Int) Signer {
	return pankuSigner{cancunSigner{londonSigner{eip2930Signer{NewEIP155Signer(chainId)}}}}
}

func (s pankuSigner) Sender(tx *Transaction) (common.Address, error) {
	if tx.Type() != PankuTxType {
...
func (s pankuSigner) Equal(o Signer) bool {
	x, ok := o.(pankuSigner)
	return ok && x.chainId.Cmp(s.chainId) == 0
}

func (s pankuSigner) SignatureValues(tx *Transaction, sig []byte) (R, S, V *big.Int, err error) {
	txdata, ok := tx.inner.(*PankuTx)
	if !ok {
...
func (s pankuSigner) Hash(tx *Transaction) common.Hash {
	if tx.Type() != PankuTxType {

...
// ! TestStateProcessorErrors测试core/error.go中定义的“core”错误输出
// 这些错误是在导入不良块时生成的，意味着块具有 有效的头 但包含 无效的交易
func TestStateProcessorErrors(t *testing.T) {
	var (
		...
		// 用于后续构造交易
		signer  = types.LatestSigner(config)                                                            // 链签名
		key1, _ = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291") // HexToECDSA解析secp256k1私钥
		key2, _ = crypto.HexToECDSA("0202020202020202020202020202020202020202020202020202002020202020")
	)
	// TODO: 定义了一个函数类型变量，作用 制造一个交易
	var makeTx = func(key *ecdsa.PrivateKey, nonce uint64, to common.Address, amount *big.Int, gasLimit uint64, gasPrice *big.Int, data []byte) *types.Transaction {
		tx, _ := types.SignTx(types.NewTransaction(nonce, to, amount, gasLimit, gasPrice, data), signer, key) // SignTx使用给定的签名者和私钥对交易进行签名
		return tx
	}
	// TODO: 定义了一个函数类型变量，作用 mkDynamicTx函数创建的是一个动态费用交易
	var mkDynamicTx = func(nonce uint64, to common.Address, gasLimit uint64, gasTipCap, gasFeeCap *big.Int) *types.Transaction {
		tx, _ := types.SignTx(types.NewTx(&types.DynamicFeeTx{
			Nonce:     nonce,
			GasTipCap: gasTipCap, // 最高gas
			GasFeeCap: gasFeeCap, // 最高小费(给矿工)
			Gas:       gasLimit,
			To:        &to,
			Value:     big.NewInt(0),
		}), signer, key1)
	...
	{ // Tests against a 'recent' chain definition
		var (
			...
			gspec = &Genesis{ // 创世区块
				Config: config,
				Alloc: GenesisAlloc{ // ? GenesisAlloc指定了初始状态，该状态是创世区块的一部分

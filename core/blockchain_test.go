...
func testBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		// statedb, err := state.New(blockchain.stateCache, state.NewHistoryDB(blockchain.db))
		if err != nil {
			return err
		}
		receipts, _, usedGas, err := blockchain.processor.Process(block, statedb, vm.Config{}) // !!! here
		if err != nil {

...
// TODO: 这个文件中的函数都是为了测试用的，实际执行中不会调用这个文件里的函数
// addTx 将交易添加到生成的块中. If no coinbase has
...
func (b *BlockGen) addTx(bc *BlockChain, vmConfig vm.Config, tx *types.Transaction) {
	...
	receipt, err, _ := ApplyTransaction(b.config, bc, &b.header.Coinbase, b.gasPool, b.statedb, b.header, tx, &b.header.GasUsed, vmConfig, true)
	if err != nil {
		panic(err)
	}
...
// AddTx 将交易添加到生成的块中。如果未设置 coinbase，则该区块的 coinbase 将设置为零地址。
// 如果交易无法执行，AddTx 会出现panic。除了协议施加的限制（gas 限制等）之外，还可以添加一些对交易内容的进一步限制。
// 值得注意的是，依赖 BLOCKHASH 指令的合约代码在执行过程中会出现panic。
...
// GenerateChain创建了一个n块的链。第一个块将具有给定的父块。db被用于存储中间状态，并且应该包含父块的state trie。

// 生成器函数为每个块调用一个新的块生成器。任何添加到生成器的交易和叔叔都将成为该块的一部分。如果gen为nil，则块将为空，其coinbase将为零地址。

// GenerateChain创建的块不包含有效的PoW值。将它们插入到BlockChain需要使用FakePow或类似的非验证性PoW实现。
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	...
	genblock := func(i int, parent *types.Block, statedb *state.StateDB) (*types.Block, types.Receipts) {
		...
		// 设置clique块的难度。该链制造商无法访问链，因此难度将被设置为未设置（nil）。在这里将其设置为正确的值。
	...
	for i := 0; i < n; i++ {
		...
		block, receipt := genblock(i, parent, statedb) // 生成区块
...
// MakeBlockChain在parent的基础上创建一个确定性的chain
...
// makeBlockChain从genesis创建确定性的chain

...
// todo deriveHash根据genesis规范计算状态根
func (ga *GenesisAlloc) deriveHash() (common.Hash, error) {
	...
	// CHANGE : commit方法已重构
	return statedb.Commit(false) // !ERROR ERROR ERROR
...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *trie.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, error) {
	...
	// 如果没有存储的创世区块，则只需提交新区块
	stored := rawdb.ReadCanonicalHash(db, 0) // ReadCanonicalHash检索分配给规范区块编号的哈希值
	if (stored == common.Hash{}) {
		if genesis == nil {
		...
		block, err := genesis.Commit(db, triedb) // !ERROR TODO:
		if err != nil {
...
// ToBlock根据genesis规范返回genesis区块
func (g *Genesis) ToBlock() *types.Block {
	root, err := g.Alloc.deriveHash() // deriveHash根据genesis规范计算状态根
	if err != nil {
...
// Commit将一个genesis规范中的区块和状态写入数据库
// 该区块被提交为规范头区块
func (g *Genesis) Commit(db ethdb.Database, triedb *trie.Database) (*types.Block, error) {
	block := g.ToBlock() // !ERROR TODO: ToBlock根据genesis规范返回genesis区块

...
func TestEIP2200(t *testing.T) {
	for i, tt := range eip2200Tests {
		...
		_, gas, err, _ := vmenv.Call(AccountRef(common.Address{}), address, nil, tt.gaspool, new(big.Int), TrueAccessList, false) // TODO:
		if err != tt.failure {
...
func TestCreateGas(t *testing.T) {
	for i, tt := range createGasTests {
		...
		doCheck := func(testGas int) bool {
			...
			TrueAccessList := NewJudgeAccessList()
			ret, gas, err, _ := vmenv.Call(AccountRef(common.Address{}), address, nil, startGas, new(big.Int), TrueAccessList, false) // TODO:
			if err != nil {
				return false
			}

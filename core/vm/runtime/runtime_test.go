...
func TestCall(t *testing.T) {
	...
	// state, _ := state.New(state.NewDatabase(rawdb.NewMemoryDatabase()), state.NewHistoryDB(rawdb.NewMemoryDatabase()))
...
func benchmarkEVM_Create(bench *testing.B, code string) {
	var (
		...
		// statedb, _ = state.New(state.NewDatabase(rawdb.NewMemoryDatabase()), state.NewHistoryDB(rawdb.NewMemoryDatabase()))
...
func benchmarkNonModifyingCode(gas uint64, code []byte, name string, tracerCode string, b *testing.B) {
	...
	// cfg.State, _ = state.New(state.NewDatabase(rawdb.NewMemoryDatabase()), state.NewHistoryDB(rawdb.NewMemoryDatabase()))
	cfg.GasLimit = gas
	if len(tracerCode) > 0 {
	...
	TrueAccessList := vm.NewJudgeAccessList()
	vmenv.Call(sender, destination, nil, gas, cfg.Value, TrueAccessList, false) // TODO:
